...
wnck_task_scale_icon (GdkPixbuf *orig, gboolean minimized)
{
  ...
  cairo_surface_t *orig_surface, *surface;
  cairo_t *cr;

  if (!orig)
    return NULL;

  orig_surface = gdk_cairo_surface_create_from_pixbuf (orig, 0, NULL);

  scaling_factor = _wnck_get_window_scaling_factor ();

  w = cairo_image_surface_get_width (orig_surface) / scaling_factor;
  h = cairo_image_surface_get_height (orig_surface) / scaling_factor;

  surface = cairo_surface_create_similar (orig_surface,
...
wnck_task_get_icon (WnckTask *task)
{
  ...
  switch (task->type)
    {
    case WNCK_TASK_CLASS_GROUP:
      surface = wnck_task_scale_icon (wnck_class_group_get_mini_icon (task->class_group),
				      FALSE);
      break;
    ...
    case WNCK_TASK_WINDOW:
      ...
      surface =  wnck_task_scale_icon (wnck_window_get_mini_icon (task->window),
          ...
          if (icon != NULL)
            {
              ...
              if (loaded != NULL)
                {
                  surface = wnck_task_scale_icon (loaded, FALSE);
